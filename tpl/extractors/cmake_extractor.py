...
from tpl.extractors.utils import *
from tpl.utils.utils import read_txt, remove_lstrip, remove_rstrip
from tpl.extractors.conan_extractor import ConanExtractor
from tpl.extractors.cpm_analyzer import cpm_func_analyzer
from tpl.extractors.hunter_analyzer import hunter_func_analyzer
...
SET_VAR_REGEX = re.compile(
    '^\\s*set\\s*\\(\\s*([a-zA-Z0-9_\\-]*)\\s+"?([a-zA-Z0-9_\\-\\.\\$\\{\\}]*)"?\\s*\\)',
    REGEX_OPTIONS,
)
        ...
        for func_body in funcs:
            ...
            for i in a:
                if not isinstance(i, cmp._Command):
                    continue
                if (
                    i.name.lower() == "pkg_check_modules"
                    or i.name.lower() == "pkg_search_module"
                ):
                    if len(i.body) == 0:
                        continue
                    for arg in i.body[1:]:
                        ...
                        if name in PKG_CHECK_MODULES_OPTIONS:
                            continue
                        name, version, opperator_op = CmakeExtractor.parse_pkg_version(
                            name
                        )
        ...
        for func_body in funcs:
            ...
            for i in a:
                if not isinstance(i, cmp._Command):
                    continue
                if (
                    i.name.lower() == "conan_cmake_run"
                    or i.name.lower() == "conan_cmake_configure"
                ):
                    if len(i.body) == 0:
                        continue
                    key_flag = 0
                    for arg in i.body:
                        if (
                            arg.contents.lower() == "requires"
                            or arg.contents.lower() == "build_requires"
                        ):
                            key_flag = 1
                            continue
                        if key_flag == 1:
                            if (
                                "/" in arg.contents
                                and arg.contents.lower() not in CONAN_CMAKE_OPTIONS
                            ):
                                ...
                                dep.add_evidence(
                                    self.type + "::conan", func_body, "High"
                                )
                                self.add_dependency(dep)
