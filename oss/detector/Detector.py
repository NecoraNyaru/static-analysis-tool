...
def detector(inputDict, inputRepo):
    ...
    path = os.path.join(resultPath, "result_" + inputRepo)
    fres = open(path, "w")
...
def detect(
    ...
    """
    Callable entry point for the detector.
    Configures globals and runs the detection logic.
    """
    global \
        ctagsPath, \
        theta, \
    ...
    repoFuncPath = os.path.join(collector_dir, "repo_functions/")
    verIDXpath = os.path.join(preprocessor_dir, "verIDX/")
    initialDBPath = os.path.join(preprocessor_dir, "initialSigs/")
    finalDBPath = os.path.join(preprocessor_dir, "componentDB/")
    metaPath = os.path.join(preprocessor_dir, "metaInfos/")
    aveFuncPath = os.path.join(metaPath, "aveFuncs")
    ...
    inputRepo = os.path.basename(target_path)
    main(target_path, inputRepo)
...
if __name__ == "__main__":
    ...
    # For standalone execution, set default relative paths
    currentPath = os.getcwd()
    resultPath = os.path.join(currentPath, "res/")
